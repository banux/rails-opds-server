...
xml.tag!("entry", {"xmlns".to_sym => "http://www.w3.org/2005/Atom", "xmlns:thr".to_sym => "http://purl.org/syndication/thread/1.0", "xmlns:dcterms".to_sym => "http://purl.org/dc/terms/", "xmlns:opds" => "http://opds-spec.org/2010/catalog"}, "xmlns:xsi".to_sym => "http://www.w3.org/2001/XMLSchema-instance") do
  ...
  if @book.author
   xml.author do |aut|
    ...
    aut.uri catalogs_all_path(:format => "atom", :auth_token => params[:auth_token], :author => @book.author)
   end
  end
  if @book.epub
  ...
  if @book.serie
   xml.link(:type => "application/atom+xml", :rel => "related", :href => catalogs_all_path(:format => "atom", :auth_token => params[:auth_token], :serie => @book.serie), :title => "From the same serie: " + @book.serie)
  end
  if @book.author
   xml.link(:type => "application/atom+xml", :rel => "related", :href => catalogs_all_path(:format => "atom", :auth_token => params[:auth_token], :author => @book.author), :title => "From the same author: " + @book.author)
  end
  if @book.category
   xml.link(:type => "application/atom+xml", :rel => "related", :href => catalogs_all_path(:format => "atom", :auth_token => params[:auth_token], :category => @book.category.id), :title => "From the same category: " + @book.category.name)
  end
end